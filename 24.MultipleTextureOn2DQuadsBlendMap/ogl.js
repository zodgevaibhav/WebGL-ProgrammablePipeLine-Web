...
var uniform_textue3_sampler;
var uniform_textue4_sampler;
...
var fragmentShaderSourceCode=
	...
	"uniform highp sampler2D u_texture3_sampler;"+
	"uniform highp sampler2D u_texture4_sampler;"+
    "out vec4 FragColor;"+
    "void main(void)"+
    "{"+
	"vec4 blenfMapColor = texture(u_texture0_sampler, vOutTexture0_cords);"+
	"float backTextureAmount = 1.0 - (blenfMapColor.r + blenfMapColor.g + blenfMapColor.b);"+
	...
	"vec4 bTextureColor=texture(u_texture3_sampler,tiledCoords) * blenfMapColor.b;"+
	"vec4 pTextureColor=texture(u_texture4_sampler,tiledCoords) * blenfMapColor.r;"+
	"vec4 totalColor = backGroupTextureColor+rTextureColor+gTextureColor+bTextureColor+pTextureColor;"+
...
//************************** Generate Textures *******************************
    ...
    quads_textureb.image.src="mud.png";
    ...
    quads_texturer.image.src="grassy2.png.png";
    ...
    quads_textureg = gl.createTexture();
    quads_textureg.image = new Image();
    quads_textureg.image.src="grassFlowers.png";
    quads_textureg.image.onload = function ()
    {
        gl.bindTexture(gl.TEXTURE_2D, quads_textureg);
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, quads_textureg.image);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.bindTexture(gl.TEXTURE_2D, null);
    }	
	
    quads_texturep = gl.createTexture();
    quads_texturep.image = new Image();
    quads_texturep.image.src="path.png";
    quads_texturep.image.onload = function ()
    {
        gl.bindTexture(gl.TEXTURE_2D, quads_texturep);
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, quads_texturep.image);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.bindTexture(gl.TEXTURE_2D, null);
    }		
	...
	//********** Bind uniforms
		...
		uniform_textue2_sampler = gl.getUniformLocation(shaderProgramObject,"u_texture3_sampler");
		uniform_textue2_sampler = gl.getUniformLocation(shaderProgramObject,"u_texture4_sampler");
...
var	modelViewMatrix=mat4.create();
	...
	gl.bindTexture(gl.TEXTURE_2D,quads_textureb);
	gl.uniform1i(uniform_textue3_sampler, 0);
	
	gl.bindTexture(gl.TEXTURE_2D,quads_texturep);
	gl.uniform1i(uniform_textue4_sampler, 0);
